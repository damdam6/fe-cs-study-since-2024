...
2️⃣ **블록 요소 (Block Element)**
    ...
    - 블록 요소 이후에 블록 요소를 사용하면 세로 형태로 표시
...
3️⃣ **인라인 요소 (Element)**
    ...
    - 인라인 요소 이후에 인라인 요소를 사용하면 가로 형태로 표시
...
**Head**
    ...
    - 메타 데이터와 타이틀을 넣어주는 곳
    - 웹 페이지에서 직접적으로 보이지는 않지만 페이지 인코딩 방식이나 검색엔진에 관한 정보 등 많은 설정이 가능
...
### HTML5의 등장, XHTML의 쇠퇴
XHTML은 HTML 4.01 버전의 element 요소를 사용하고, 문법은 XML을 따른다고 한다. HTML5 등장 이후, 여전히 HTML은 태그 닫는 것을 선택사항으로 두지만 오류를 야기하지 않도록 한다. 따라서 코드를 적게 사용하더라도 브라우저에서 모든 태그를 인식하게 함으로써 웹사이트의 크기 감량과 빠른 코딩이 가능해졌다. HTML5가 정립된 이후 문서 중심인 XHTML은 사용되는 일이 많이 없다고 한다.
